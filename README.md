...
b. Buat recurring series (contoh Senin dan Rabu jam 09:00)
```bash
...
c. Lihat daftar recurring series
```bash
curl "http://localhost:3000/api/events/recurring?page=1&pageSize=10"
```

d. Edit satu occurrence (cancel satu tanggal)
```bash
...
e. Edit series ke depan dari tanggal tertentu
```bash
...
f. Hapus satu occurrence
```bash
curl -X DELETE http://localhost:3000/api/events/recurring/<SERIES_ID>/occurrences/<EVENT_ID> \
  -H "Authorization: Bearer <TOKEN_ADMIN>"
```

g. Hapus seluruh future occurrences dalam satu series (past occurrence tetap)
```bash
